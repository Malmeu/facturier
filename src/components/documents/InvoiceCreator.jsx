...
const InvoiceCreator = ({ currentLang, languages }) => {
  ...
  const addItem = () => {
    const newItem = {
      id: Date.now(),
      title: '', // Titre de l'article (ex: "Prestation centre d'appels")
      description: '', // Description détaillée
      type: 'standard', // Type: standard, horaire, forfait, contrat, rendez-vous
      quantity: 1,
      unit: 'unité', // Unité: unité, heure, jour, forfait, rendez-vous
      unitPrice: 0,
      period: '', // Période concernée (ex: "Janvier 2025" ou "Semaine 36")
      reference: '', // Référence du contrat ou du service
      total: 0
    }
    setInvoice(prev => ({
      ...prev,
              ...
              <div className="space-y-4">
                {invoice.items.map((item, index) => (
                  <div key={item.id} className="bg-gray-50 rounded-md p-4 border border-gray-200">
                    ...
                    <div className="grid grid-cols-12 gap-3 mb-3">
                      <div className="col-span-6">
                        ...
                        <input
                          ...
                          placeholder="Ex: Prestation centre d'appels"
                    ...
                    {/* Troisième ligne: Quantité, Unité, Prix unitaire */}
                    <div className="grid grid-cols-12 gap-3 mb-3">
                    ...
                    {/* Quatrième ligne: Période et Référence */}
                    <div className="grid grid-cols-12 gap-3">
                      <div className="col-span-6">
                        ...
                        <input
                          type="text"
                          value={item.period || ''}
                          onChange={(e) => updateItemField(index, 'period', e.target.value)}
                          placeholder="Ex: Janvier 2025 ou Semaine 36"
                          className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                      </div>
                      <div className="col-span-6">
                        ...
                        <input
                          ...
                          onChange={(e) => updateItemField(index, 'reference', e.target.value)}
                          placeholder="Ex: Contrat #12345 ou Projet XYZ"
                          className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                      </div>
                    </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      ...
                      >
                        ...
                        <th className="text-center py-3 px-2">Unité</th>
                    ...
                    <tbody>
                      {invoice.items.map((item, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                                ...
                                {item.type && item.type !== 'standard' && (
                                  <span className="inline-flex items-center px-2 py-1 rounded-full bg-blue-100 text-blue-800">
                                    {item.type === 'horaire' ? 'Horaire' : 
                                     ...
                                     item.type === 'personnalise' ? 'Personnalisé' : item.type}
                                  </span>
                                )}
                                {item.period && (
                          ...
                          <td className="py-3 px-2 text-center text-gray-600">{item.unit || 'unité'}</td>
                          <td className="py-3 px-2 text-right font-medium">{item.unitPrice.toFixed(2)}</td>
                          <td className="py-3 px-2 text-right font-bold text-gray-900">{item.total.toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
